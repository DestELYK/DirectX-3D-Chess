...
void PawnPiece::Initiialize(ID3D11Device* pDevice, LitColourShader* pLitShader)
{
	pShader = pLitShader;
	...
	bodyMatrix = Matrix::CreateScale(Vector3(1.2f, 2.4f, 1.2f));
	topMatrix = Matrix::CreateScale(1.2f) * Matrix::CreateTranslation(0, 0.6f, 0);
}

// called to draw the object
// The parent matrix allows the user to pass in a matrix to transform the entire piece
void PawnPiece::Draw(ID3D11DeviceContext* pDeviceContext, const Matrix& parentMatrix, const Matrix& viewMatrix, const Matrix& projMatrix)
{
...
// update the object
void PawnPiece::Update(float deltaTime)
{
}